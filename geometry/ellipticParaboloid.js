export class ellipticParaboloid {
    constructor(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, _normal, opts = {}) {
        ...
        const cap = opts.cap ?? true; // Tambahkan tutup atas?
    ...
    _build(a, b, c, height, segments, rings, color, addCap) {
        ...
        for (let i = 0; i <= rings; i++) {
            ...
            for (let j = 0; j <= segments; j++) {
                ...
                // Hitung Normal: N = normalize((-2x/a^2, 1/c, -2z/b^2))
                let nx_raw = -2 * x / (a * a);
                let ny_raw = 1 / c;
                let nz_raw = -2 * z / (b * b);
    ...
    render(PARENT_MATRIX) {
        // Render function sama persis seperti ellipsoid.js
        const M = LIBS.get_I4();
        ...
        this.GL.enableVertexAttribArray(this._position); // Aktifkan posisi
        this.GL.vertexAttribPointer(this._color, 3, this.GL.FLOAT, false, 36, 12);
        this.GL.enableVertexAttribArray(this._color);    // Aktifkan warna
        this.GL.vertexAttribPointer(this._normal, 3, this.GL.FLOAT, false, 36, 24);
        this.GL.enableVertexAttribArray(this._normal);   // Aktifkan normal
