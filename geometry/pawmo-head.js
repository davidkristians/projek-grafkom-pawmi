...
export class pawmoHead extends group {
    constructor(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, _normal, opts = {}) {
        ...
        // --- 2. Pembuatan Objek Kepala, Moncong, Hidung, Mata, Mulut, Pipi ---

        // Bagian Dasar Kepala
        const baseHead = new ellipsoid(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, _normal, {
            rx: 1.2, ry: 1.1, rz: 1.1, color: PAWMO_ORANGE
        });

        // Bagian Moncong
        const muzzle = new ellipsoid(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, _normal, {
            rx: 0.6, ry: 0.45, rz: 0.6, color: PAWMO_CREAM
        });

        // Bagian Hidung
        const nose = new ellipsoid(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, _normal, {
            rx: 0.05, ry: 0.05, rz: 0.05, color: PAWMO_BLACK
        });

        // Bagian Mata
        const leftEye = new ellipsoid(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, _normal, {
            rx: 0.2, ry: 0.2, rz: 0.2, color: PAWMO_BLACK
        });
        const rightEye = new ellipsoid(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, _normal, {
            rx: 0.2, ry: 0.2, rz: 0.2, color: PAWMO_BLACK
        });
        const leftEyepupil = new ellipsoid(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, _normal, {
            rx: 0.05, ry: 0.05, rz: 0.01, color: PAWMO_WHITE
        });
        const rightEyepupil = new ellipsoid(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, _normal, {
            rx: 0.05, ry: 0.05, rz: 0.01, color: PAWMO_WHITE
        });
        ...
        const smile = new ellipsoid(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, _normal, {
            rx: 0.2,
            ry: 0.001, rz: 0.03, color: PAWMO_BLACK
        });

        // Bagian Pipi
        const leftCheek = new ellipsoid(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, _normal, {
            rx: 0.4, ry: 0.35, rz: 0.1, color: PAWMO_YELLOW
        });
        const rightCheek = new ellipsoid(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, _normal, {
            rx: 0.4, ry: 0.35, rz: 0.1, color: PAWMO_YELLOW
        });

        // Area Telinga Kiri
        const leftEar = new pawmoEar(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, _normal, {
            orange: PAWMO_ORANGE
        });
        ...
        // Area Telinga Kanan
        const rightEar = new pawmoEar(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, _normal, {
            orange: PAWMO_ORANGE
        });
        ...
        // Tuft (Jambul)
        const headTuft = new pawmoTuft(GL, SHADER_PROGRAM, _position, _color, _Mmatrix, _normal, {
            color: PAWMO_ORANGE
        });


        // --- 3. Transformasi Objek Moncong, Hidung, Mata, Mulut, Pipi ---
        LIBS.set_I4(muzzle.POSITION_MATRIX);
        LIBS.translateZ(muzzle.POSITION_MATRIX, 0.6);
        LIBS.translateY(muzzle.POSITION_MATRIX, -0.2);

        // --- Posisi Hidung ---
        LIBS.set_I4(nose.POSITION_MATRIX);
        LIBS.translateZ(nose.POSITION_MATRIX, 1.2);
        LIBS.translateY(nose.POSITION_MATRIX, -0.1);
        ...
        LIBS.translateY(smile.POSITION_MATRIX, -0.3);
        LIBS.translateX(smile.POSITION_MATRIX, 0.0);
        LIBS.translateZ(smile.POSITION_MATRIX, 1.18);
        LIBS.rotateX(smile.POSITION_MATRIX, LIBS.degToRad(50));
        ...
        // --- Posisi Telinga ---
        // Telinga Luar Kiri
        LIBS.set_I4(leftEar.POSITION_MATRIX);
        LIBS.translateX(leftEar.POSITION_MATRIX, -0.9);
        LIBS.translateY(leftEar.POSITION_MATRIX, 1.2);
        LIBS.translateZ(leftEar.POSITION_MATRIX, 0.2);
        LIBS.rotateY(leftEar.POSITION_MATRIX, LIBS.degToRad(90));
        LIBS.rotateX(leftEar.POSITION_MATRIX, LIBS.degToRad(20));
        LIBS.rotateZ(leftEar.POSITION_MATRIX, LIBS.degToRad(30));

        // Telinga Dalam Kiri (Ellipsoid)
        ...
        // Telinga Luar Kanan
        LIBS.set_I4(rightEar.POSITION_MATRIX);
        LIBS.translateX(rightEar.POSITION_MATRIX, 0.9);
        LIBS.translateY(rightEar.POSITION_MATRIX, 1.2);
        LIBS.translateZ(rightEar.POSITION_MATRIX, 0.2);
        LIBS.rotateY(rightEar.POSITION_MATRIX, LIBS.degToRad(-90));
        LIBS.rotateX(rightEar.POSITION_MATRIX, LIBS.degToRad(20));
        LIBS.rotateZ(rightEar.POSITION_MATRIX, LIBS.degToRad(-30));

        // Telinga Dalam Kanan (Ellipsoid)
